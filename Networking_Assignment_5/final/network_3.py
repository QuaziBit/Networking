...
class NetworkPacket:
    ...
    def __init__(self, dst, data_S, priority=0):
        ...
        # Add priority to the packet
        # and check packet for an extra priority numbers, sometime it can have more than one priority number
        # ============================================================================== #
        # a. [2 points] Recall that the IP header has a type of service (TOS) field that carries packet priority. 
        """print("\n\t\t ???=> PACKET BEFORE PRIORITY: %s <=???\n" % (self.data_S))"""
        if self.data_S[0] is not "0" and self.data_S[0] is not "1":
            tmp_s = '%d%s' % (self.priority, self.data_S)
            self.data_S = tmp_s
        """print("\n\t\t @@@=> FINAL PACKET WITH PRIORITY: %s <=@@@\n" % (self.data_S))"""
        # ============================================================================== #
