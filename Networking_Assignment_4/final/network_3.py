...
class Router:
    ...
    def forward_packet(self, p):
        router = list(self.rt_tbl_D[p.dst].keys())[0]  # Outgoing Router
        if router == self.name:
            out_interface = self.fwd_tbl_D[p.dst]
        else:
            out_interface = self.fwd_tbl_D[router]
        try:
            self.intf_L[out_interface].put(p.to_byte_S(), 'out', True)
            if router != self.name:
                print('\n\t\t*** forward_packet "%s" from "%s" to "%s" ***' % (p, self, router))
            else:
                print('\n\t\t*** forward_packet "%s" from "%s" to "%s" ***' % (p, self, p.dst))
            print('\t\t\t\t\t\t*** from interface %d ***\n' % out_interface)
        except queue.Full:
            print('%s: packet "%s" lost on interface %d' % (self, p, out_interface))
            pass

    # send out route update
    # Interface for target neighbor taken from cost_D
    def send_routes(self):
        for v, value in self.rt_tbl_D.items():
            target = list(value.keys())[0]
            # Only broadcast route updates to routers, not hosts. Assumes that all routers will start with 'R'
            if v[0:1] == 'R' and v != self.name:
                p = NetworkPacket(v, 'control', self.name + json.dumps(self.rt_tbl_D))
                if target == self.name:
                    out_interface = self.fwd_tbl_D[p.dst]
                else:
                    out_interface = self.fwd_tbl_D[target]
                try:
                    print('%s: sending routing update "%s" from interface %d' % (self, p, out_interface))
                    self.intf_L[out_interface].put(p.to_byte_S(), 'out', True)
                except queue.Full:
                    print('%s: packet "%s" lost on interface %d' % (self, p, out_interface))
                    pass
    ...
    def bellman_ford(self, router, update_D):
        # Have to take key value because the shortest path may be to use another value,
        # so self.rt_tbl_D[router][router] did not exist
        dist_router = list(self.rt_tbl_D[router].values())[0]
            ...
            if self.name == vertex:
                ...
                if vertex not in self.rt_tbl_D:
                    ...
                    if dist_u[0] > (dist_router + dist_v[0]):
                        if router != 'RD':
                            self.rt_tbl_D[vertex] = {router: (dist_router + dist_v[0])}
                        else:
                            self.rt_tbl_D[vertex] = {'RC': (dist_router + dist_v[0])}
    ...
    def update_routes(self, p, i):
        # To forward control packets sent between unconnected routers
        if p.dst != self.name:
            self.forward_packet(p)
        else:
